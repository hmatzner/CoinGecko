...
class Database:
    ...
    def update_wallets(self, data):
        ...
        data_to_append = pd.merge(data, existing_rows, on=data.columns.to_list(), how='left', indicator=True).query(
                                "_merge != 'both'").drop('_merge', axis=1).reset_index(drop=True)

        data_to_append = data_to_append.to_dict('records')
        query = """INSERT INTO wallets (coin_id, wallet_id)
                            VALUES (%(coin_id)s, %(wallet_id)s)"""

        try:
            self.cursor.executemany(query, data_to_append)
            self.connection.commit()
            self.logger.info(f"{len(data_to_append)} rows were successfully uploaded to the wallets table")
        except Exception as e:
            self.logger.info(e)
